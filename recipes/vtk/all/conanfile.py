...
class VtkConan(ConanFile):
    ...
    def _configure_cmake(self):
        if not self._cmake:
            ...
            self._cmake.definitions["VTK_SMP_IMPLEMENTATION_TYPE"] = self._convert_smp_option(
                self.options.vtk_smp_implementation)
            self._cmake.definitions["VTK_WRAP_PYTHON"] = "ON" if self.options.vtk_wrap_python else "OFF"
            self._cmake.definitions["VTK_USE_CUDA"] = "ON" if self.options.vtk_use_cuda else "OFF"
            self._cmake.definitions["VTK_GROUP_ENABLE_Imaging"] = self._convert_vtk_choice(
                self.options.vtk_group_enable_imaging)
            self._cmake.definitions["VTK_GROUP_ENABLE_Qt"] = self._convert_vtk_choice(
                self.options.vtk_group_enable_qt)
            self._cmake.definitions["VTK_GROUP_ENABLE_Rendering"] = self._convert_vtk_choice(
                self.options.vtk_group_enable_rendering)
            self._cmake.definitions["VTK_GROUP_ENABLE_MPI"] = self._convert_vtk_choice(
                self.options.vtk_group_enable_mpi)
            self._cmake.definitions["VTK_GROUP_ENABLE_StandAlone"] = self._convert_vtk_choice(
                self.options.vtk_group_enable_standalone)
            self._cmake.definitions["VTK_GROUP_ENABLE_Views"] = self._convert_vtk_choice(
                self.options.vtk_group_enable_views)
            self._cmake.definitions["VTK_GROUP_ENABLE_Web"] = self._convert_vtk_choice(
                self.options.vtk_group_enable_web)
            self._cmake.definitions["VTK_IOS_BUILD"] = "OFF"
            self._cmake.definitions["VTK_WRAP_JAVA"] = "OFF"

            if self.options.vtk_wrap_python and self.options.build_wheel:
    ...
    def package_info(self):
        ...
        base_include_dir = os.path.join(
            'include', 'vtk-%d.%d' % (pkg_ver[0], pkg_ver[1]))
        ...
        for name, info in enabled_pkg.items():
            ...
            self.cpp_info.components[alias].libs = [
                '%s-%d.%d' % (lib_name, pkg_ver[0], pkg_ver[1])]
            dep = self.filter_dep(name, content['depends'], enabled_pkg)
            opt_dep = self.filter_dep(
                name, content['private_depends'], enabled_pkg)
            ...
            if name == "VTK::RenderingOpenGL2":
                self.cpp_info.components[alias].requires.append(
                    "opengl::opengl")
            ...
            if self.settings.os == 'Linux':
                ...
                if name == "VTK::vtksys":
                    # 'libvtksys-7.1.a' require 'dlclose', 'dlopen', 'dlsym' and 'dlerror' which on CentOS are in 'dl' library
                    self.cpp_info.components[alias].system_libs.append('dl')

            if not self.options.shared and name == "VTK::GUISupportQt":
                if self.settings.os == 'Windows':
                    # 'vtksys-9.0d.lib' require 'gethostbyname', 'gethostname', 'WSAStartup' and 'WSACleanup' which are in 'Ws2_32.lib' library
                    self.cpp_info.components[alias].system_libs.append(
                        'ws2_32')
                    # 'vtksys-9.0d.lib' require 'GetProcessMemoryInfo' which is in 'Psapi.lib' library
                    self.cpp_info.components[alias].system_libs.append('psapi')
                    # 'vtksys-9.0d.lib' require '__imp_SymGetLineFromAddr64', '__imp_SymInitialize' and '__imp_SymFromAddr' which are in 'dbghelp.lib' library
                    self.cpp_info.components[alias].system_libs.append(
                        'dbghelp')

                if self.settings.os == 'Macos':
                    # 'libvtkRenderingOpenGL2-9.0.a' require '_CFRelease', '_CFRetain', '_objc_msgSend' and much more which are in 'CoreFoundation' library
                    self.cpp_info.components[alias].frameworks.extend(
                        ["corefoundation"])
                    # 'libvtkRenderingOpenGL2-9.0.a' require '_CGWarpMouseCursorPosition' and more, 'libvtkRenderingUI-9.0.a' require '_OBJC_CLASS_$_NSApplication' and more, which are in 'Cocoa' library
                    self.cpp_info.components[alias].frameworks.extend([
                                                                      "cocoa"])
